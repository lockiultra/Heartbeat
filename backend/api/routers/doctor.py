...
from backend.api.schemas.doctor import * # noqa
...
@doctor_router.get('/{id:int}')
async def get_doctor_by_id(id: int, db: AsyncSession = Depends(get_db)):
    return await DoctorRepository.get_by_id(db=db, id=id)

@doctor_router.post('/')
async def create_doctor(create_scheme: DoctorCreate = Depends(), db: AsyncSession = Depends(get_db)):
    return await DoctorRepository.create(db=db, create_scheme=create_scheme)

@doctor_router.put('/')
async def update_doctor(update_scheme: DoctorUpdate = Depends(), db: AsyncSession = Depends(get_db)):
    return await DoctorRepository.update(db=db, update_scheme=update_scheme)

@doctor_router.delete('/')
async def delete_doctor(delete_scheme: DoctorDelete = Depends(), db: AsyncSession = Depends(get_db)):
